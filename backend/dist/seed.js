...
async function main() {
    ...
    if (seedSampleData) {
        ...
        for (const outcomeName of outcomeNames) {
            const existing = await context_1.prisma.outcome.findFirst({ where: { name: outcomeName, productId: product.id } });
            if (!existing) {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            const existing = await context_1.prisma.task.findFirst({ where: { name: taskName, productId: product.id } });
            if (!existing) {
